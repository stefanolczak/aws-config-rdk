...
class Init:
    ...
    def run(self):
        ...
        self.args = Init.get_init_parser().parse_args(self.args.command_args, self.args)
        ...
        if self.args.config_bucket_exists_in_another_account:
            print(f"[{my_session.region_name}]: Skipping Config Bucket check due to command line self.args")
        ...
        if self.args.generate_lambda_layer:
            lambda_layer_version = util.get_existing_lambda_layer(my_session, layer_name=self.args.custom_layer_name)
            ...
            util.create_new_lambda_layer(my_session, layer_name=self.args.custom_layer_name)
            lambda_layer_version = util.get_existing_lambda_layer(my_session, layer_name=self.args.custom_layer_name)
        ...
        if not bucket_exists:
            if self.args.skip_code_bucket_creation:
                print(f"[{my_session.region_name}]: Skipping Code Bucket creation due to command line self.args")
            else:
