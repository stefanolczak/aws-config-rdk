...
def deploy(args):
    args = get_deployment_parser().parse_args(args.command_args, args)
    ...
    if args.lambda_layers:
        ...
        if layer_count > 5:
            ...
            sys.exit(1)
        if args.rdklib_layer_arn or args.generated_lambda_layer and layer_count > 4:
    ...
    if "rdk_lib_version" in args and (args.rdklib_layer_arn or args.generated_lambda_layer):
        print(
    ...
    # RDKLib version and RDKLib Layer ARN/Generated RDKLib Layer are mutually exclusive.
    if args.rdklib_layer_arn and args.generated_lambda_layer:
    ...
    # run the deploy code
    print(f"[{args.region}]: Running deploy!")
    ...
    if args.functions_only:
        ...
        function_template = Util.create_function_cloudformation_template(args)
        ...
        try:
            my_stack = my_cfn.describe_stacks(StackName=args.stack_name)
            ...
            try:
                ...
                Util.wait_for_cfn_stack(my_cfn, args.stack_name, args)
    ...
    for rule_name in rule_names:
        ...
        if "SourceIdentifier" in rule_params:
            ...
            if "Remediation" in rule_params:
                ...
                # wait for changes to propagate.
                Util.wait_for_cfn_stack(my_cfn, my_stack_name, args)
                continue
                ...
                try:
                    ...
                    my_stack = my_cfn.describe_stacks(StackName=my_stack_name)
                    # If we've gotten here, stack exists and we should update it.
                    print(f"[{args.region}]: Updating CloudFormation Stack for " + rule_name)
                    try:
                        cfn_args = {
                            "StackName": my_stack_name,
                            "TemplateBody": open(cfn_body, "r").read(),
                    ...
                    # If we're in the exception, the stack does not exist and we should create it.
                    print(f"[{args.region}]: Creating CloudFormation Stack for " + rule_name)
                    cfn_args = {
                        "StackName": my_stack_name,
                        "TemplateBody": open(cfn_body, "r").read(),
                        "Parameters": my_params,
            ...
            if cfn_tags is not None and len(cfn_tags) > 0:
                Util.tag_config_rule(rule_name, cfn_tags, my_session, args)
        ...
        if args.lambda_role_arn:
            print(f"[{my_session.region_name}]: Existing IAM Role provided: " + args.lambda_role_arn)
        ...
        if args.boundary_policy_arn:
            print(f"[{my_session.region_name}]: Boundary Policy provided: " + args.boundary_policy_arn)
        ...
        if "SSMAutomation" in rule_params:
            ...
            json_body["Resources"][Util.get_alphanumeric_rule_name(rule_name + "RemediationAction")] = ssm_automation
            if "IAM" in rule_params["SSMAutomation"]:
        ...
        try:
            ...
            my_lambda_arn = Util.get_lambda_arn_for_stack(my_stack_name, args)
